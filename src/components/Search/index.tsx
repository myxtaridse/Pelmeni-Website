...
import { setIsSearch } from "../../redux/filterSlice/sliceFilter";
import debounce from "lodash.debounce";
...
const Search: React.FC = () => {
  ...
  //const isSearch = useSelector((state) => state.itemsSlice);
  ...
  const updateSearch = React.useCallback(
    ...
    debounce((str: string) => {
      dispatch(setIsSearch(str)); //совершает отложенное выполнение функции, через секунду
    }, 1000),
    []
  );

  // event - событие (ChangeEvent) какого-то HTML-элемента (HTMLInputElement)
  // Handler - некая функция, которая отлавливает события,
  //событие изменения - ChangeEventHandler
  //событие клика - MouseEventHandler
